...
const AddVehiclePage = () => {
	const vehicleForm = useForm({
		...
		validate: {
			plateNumber: (value, values) =>
				values.vehicleType.toLowerCase() === 'bus' && value === ''
					? 'This field cannot be left empty'
					: null,
			tailNumber: (value, values) =>
				values.vehicleType.toLowerCase() === 'plane' && value === ''
					? 'This field cannot be left empty'
					: null,
			vehicleType: (value) =>
				value === '' ||
				(value.toLowerCase() != 'bus' && value.toLowerCase() != 'plane')
					? 'This field should be Bus or Plane'
					: null,
			capacity: (value) => (value == 0 ? 'This field cannot be left empty' : null),
			configName: (value) =>
